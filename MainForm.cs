...
{
    private const string AppTitle = "HamLogger Contact Editor  Version 1.1";
    ...
    private MenuStrip BuildMenu()
    {
        ...
        file.DropDownItems.Add(Item("&New", Keys.Control | Keys.N, NewFile));
        file.DropDownItems.Add(Item("&Open ADIF...", Keys.Control | Keys.O, OpenFile));
        file.DropDownItems.Add(Item("&Merge ADIF File...", Keys.Control | Keys.M, MergeFile));
        file.DropDownItems.Add(new ToolStripSeparator());
        file.DropDownItems.Add(Item("&Save", Keys.Control | Keys.S, () => SaveFile(false)));
        file.DropDownItems.Add(Item("Save &As...", Keys.Control | Keys.Shift | Keys.S, () => SaveFile(true)));
        file.DropDownItems.Add(Item("Save Se&lected Rows As...", Keys.None, SaveSelectedRows));
        file.DropDownItems.Add(new ToolStripSeparator());
        var export = new ToolStripMenuItem("&Export");
        export.DropDownItems.Add(Item("&CSV File...", Keys.None, ExportCsv));
        export.DropDownItems.Add(Item("S&QLite INSERT Script...", Keys.None, () => ExportSql(SqlDialect.SQLite)));
        export.DropDownItems.Add(Item("SQL &Server INSERT Script...", Keys.None, () => ExportSql(SqlDialect.SqlServer)));
    ...
    private ToolStrip BuildToolBar()
    {
        ...
        bar.Items.Add(Button("Open ADIF", "Open an ADIF file; its field names become the columns", OpenFile));
        bar.Items.Add(Button("Merge File", "Add the rows of another ADIF file (tagged with its file name)", MergeFile));
        bar.Items.Add(Button("Save ADIF", "Save all rows", () => SaveFile(false)));
        bar.Items.Add(Button("Save Selected", "Save only the selected rows to a new ADIF file", SaveSelectedRows));
        ...
        bar.Items.Add(new ToolStripSeparator());
        bar.Items.Add(Button("Export CSV", "Export all rows to a CSV file", ExportCsv));
        bar.Items.Add(Button("SQLite Script", "Create a SQLite INSERT script", () => ExportSql(SqlDialect.SQLite)));
        bar.Items.Add(Button("SQL Server Script", "Create a SQL Server INSERT script", () => ExportSql(SqlDialect.SqlServer)));
        return bar;
    }
    ...
    private static ToolStripMenuItem Item(string text, Keys keys, Action action)
    {
    ...
    private static ToolStripButton Button(string text, string tip, Action action) =>
        new(text, null, (_, _) => action()) { DisplayStyle = ToolStripItemDisplayStyle.Text, ToolTipText = tip };
    ...
    private void RebuildGrid(IReadOnlyList<ColumnSpec> layout, bool keepSortAndFilter = true, Action? schemaChange = null)
    {
        ...
        string sort = keepSortAndFilter ? table.DefaultView.Sort ?? string.Empty : string.Empty;
        string filter = keepSortAndFilter ? table.DefaultView.RowFilter ?? string.Empty : string.Empty;
        int sourceWidth = grid.Columns.Contains(SourceColumn) ? grid.Columns[SourceColumn].Width : 160;
    ...
    private void OpenPath(string path)
    {
        ...
        {
            UseWaitCursor = true;
            AdifLog log = AdifFile.Load(path);
            ...
            foreach (AdifRecord record in log.Records) AddRecord(newTable, record, null, null);
            newTable.EndLoadData();

            LoadTable(newTable, autoSize: true);
            currentPath = path;
            dirty = mergedSinceSave = false;
            UpdateTitle();
            UpdateStatus($"Loaded {log.Records.Count:N0} contacts with {fields.Count} fields from {Path.GetFileName(currentPath)}");
        }
        catch (Exception ex)
        {
    ...
    private static void AddRecord(DataTable t, AdifRecord record, IReadOnlyDictionary<string, string>? fieldMap, string? sourceFile)
    {
    ...
    private void MergeFile()
    {
        ...
        AdifLog log;
        try { log = AdifFile.Load(dialog.FileName); }
        catch (Exception ex)
        {
            MessageBox.Show(this, ex.Message, "Unable to open ADIF", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        ...
        if (unknown.Count > 0)
        {
            ...
            if (answer == DialogResult.Yes)
            {
                foreach (string field in unknown)
                {
                    ...
                    layout.Add(new ColumnSpec(null, name, 100));
                    fieldMap[field] = name;
                }
            }
        }
        ...
        RebuildGrid(layout, schemaChange: () =>
        {
            if (table.Columns.Contains(SourceColumn)) return;
            DataColumn column = table.Columns.Add(SourceColumn, typeof(string));
            string original = currentPath is null ? "(original)" : Path.GetFileName(currentPath);
            foreach (DataRow row in table.Rows) row[column] = original;
        });
        ...
        Bulk(() =>
        {
            ...
            {
                table.BeginLoadData();
                foreach (AdifRecord record in log.Records) AddRecord(table, record, fieldMap, fileName);
                table.EndLoadData();
            }
        ...
        MarkDirty();
        UpdateStatus($"Merged {log.Records.Count:N0} contacts from {fileName}");
    }
    ...
    private bool SaveFile(bool saveAs)
    {
        ...
        try
        {
            ...
            AdifFile.Save(path, columns, RowValues(rows, columns));
            currentPath = path;
            dirty = mergedSinceSave = false;
            UpdateTitle();
            UpdateStatus($"Saved {rows.Count:N0} contacts to {Path.GetFileName(path)}");
            return true;
        }
        catch (Exception ex)
        {
    ...
    private void SaveSelectedRows()
    {
        ...
        try
        {
            var columns = CurrentLayout().Select(c => c.Name).ToList();
            AdifFile.Save(path, columns, RowValues(rows, columns));
            UpdateStatus($"Saved {rows.Count:N0} selected contacts to {Path.GetFileName(path)}");
        }
        catch (Exception ex)
        {
            MessageBox.Show(this, ex.Message, "Unable to save ADIF", MessageBoxButtons.OK, MessageBoxIcon.Error);
    ...
    private void ExportCsv()
    {
        ...
        try
        {
            ...
            var rows = AllRowsInSortOrder();
            CsvExporter.Save(dialog.FileName, headers, RowValues(rows, columns));
            UpdateStatus($"Exported {rows.Count:N0} rows to {Path.GetFileName(dialog.FileName)}");
        }
        catch (Exception ex)
        {
            MessageBox.Show(this, ex.Message, "Unable to export CSV", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }

    private void ExportSql(SqlDialect dialect)
    {
        grid.EndEdit();
        ...
        try
        {
            var (headers, columns) = ExportColumns();
            var rows = AllRowsInSortOrder();
            SqlScriptExporter.Save(dialog.FileName, dialect, tableName, headers, RowValues(rows, columns));
            UpdateStatus($"Wrote {dbName} script for {rows.Count:N0} rows to {Path.GetFileName(dialog.FileName)}");
        }
        catch (Exception ex)
        {
            MessageBox.Show(this, ex.Message, $"Unable to create {dbName} script", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    ...
    private bool ConfirmDiscard()
    {
        ...
        {
            DialogResult.Yes => SaveFile(false),
    ...
    private void AddContact()
    {
        ...
        DataGridViewRow? gridRow = grid.Rows.Cast<DataGridViewRow>()
            .FirstOrDefault(r => (r.DataBoundItem as DataRowView)?.Row == row);
    ...
    {
        searchDialog ??= new SearchReplaceDialog(grid, UpdateStatus);
        searchDialog.ShowFor(this, null);
    }
